<script lang="ts">
  ...
  const SCRIPT = [
    ...
    { delay: 2100, line: '-rw-r--r--  myterm/  go-api/  svelte-app/', type: 'out' },
  ...
  onMount(() => {
    ...
    const blinkInterval = setInterval(() => { showCursor = !showCursor; }, 550);
...
<div class="mock-wrap">
  ...
  <div class="titlebar">
    ...
    <div class="tabs">
      <button class="tab active">bash</button>
      <button class="tab">pwsh</button>
      ...
      {#if line.type === 'prompt'}
        <div class="line">
          ...
          {#if i === visibleLines.length - 1}
            <span class="cursor-beam" style="opacity:{showCursor ? 1 : 0}">▋</span>
          {/if}
        </div>
      {:else if line.type === 'cmd'}
        <div class="line"><span class="ps1">~/projects</span><span class="ps-arrow"> › </span><span class="cmd-text">{line.text}</span></div>
  ...
  <div class="split-bar" aria-hidden="true">
    <span class="split-info">Split: <kbd>Ctrl+Shift+E</kbd></span>
    <span class="split-info">New tab: <kbd>Ctrl+T</kbd></span>
    <span class="split-info">Copy: <kbd>Ctrl+Shift+C</kbd></span>
  </div>
</div>
...
<style>
  ...
  .tab.dim { opacity: 0.4; }
  .tab:hover:not(.active) { background: color-mix(in srgb, var(--bg) 50%, var(--dim)); }
  ...
  .ps1    { color: var(--cyan); }
  .ps-arrow { color: var(--accent); margin: 0 2px; }
  .cmd-text { color: var(--fg); }
  .cursor-beam { color: var(--accent); transition: opacity 0.05s; }
  .out    { color: var(--fg); padding-left: 4px; }
  .out-dim  { color: var(--br-black); padding-left: 4px; }
